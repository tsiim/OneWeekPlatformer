...
function OnControllerColliderHit (hit : ControllerColliderHit) {
	...
	// **** I added to check if the player hits a killing object
	if(body.tag == ("Danger"))
		this.SendMessage("OnDeath");
	var pushDir = new Vector3();
	var massDif = 1/body.mass;
	...
	// **** I added the Y direction as i want the player to move objects up
	pushDir = Vector3 (hit.moveDirection.x, hit.moveDirection.y, hit.moveDirection.z);
	
	// push with move speed but never more than walkspeed
	body.velocity = pushDir * pushPower * Mathf.Min (controller.GetSpeed (), controller.movement.walkSpeed)*massDif;
}
